    ...
    async def get_available_slots(
        ...
        try:
            ...
            for event in events:
                ...
                # Redondear al slot más cercano de 30 min
                minute = (event_start_local.minute // duration_minutes) * duration_minutes
                slot_time = event_start_local.replace(minute=minute, second=0, microsecond=0)
                busy_slots.add(slot_time)
            ...
            logger.error("Error calculando slots", error=str(e), date=date)
            raise
