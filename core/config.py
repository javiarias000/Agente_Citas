...
class Settings(BaseSettings):
    ...
    # Memory-Agent Integration (memoria semántica vectorial)
    USE_MEMORY_AGENT: bool = Field(
        default=False,
        description="Habilitar integración con memory-agent para memorias semánticas"
    )
    MEMORY_AGENT_STORE_TYPE: str = Field(
        default="memory",
        description="Tipo de store: 'memory' (InMemoryStore) o 'postgres' (PostgreSQLStore)"
    )
    MEMORY_AGENT_STORE_URL: Optional[str] = Field(
        default=None,
        description="URL para PostgreSQLStore (si STORE_TYPE=postgres). Si None, usa DATABASE_URL"
    )
    MEMORY_AGENT_EMBEDDING_MODEL: str = Field(
        default="openai:text-embedding-3-small",
        description="Modelo de embeddings (formato: provider:model_name)"
    )
    MEMORY_AGENT_MAX_RESULTS: int = Field(
        default=5,
        description="Número máximo de resultados en búsqueda semántica"
    )
    MEMORY_AGENT_SIMILARITY_THRESHOLD: float = Field(
        default=0.7,
        description="Umbral de similitud para búsqueda (0-1)"
    )
    MEMORY_AGENT_ENABLE_EXTRACTION: bool = Field(
        default=False,
        description="Extracción automática de hechos (experimental)"
    )
    MEMORY_AGENT_SYNC_TO_USER_PROFILE: bool = Field(
        default=True,
        description="Sincronizar memorias con tabla user_profiles.extracted_facts"
    )
