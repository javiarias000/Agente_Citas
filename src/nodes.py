        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                return {
                    ...
                    "should_escalate": kwargs.get("state", {}).get("errors_count", 0) >= 2,
                }
...
async def node_entry(
    ...
    *,
    ...
    - Carga historial y estado previos del store
    - Incrementa conversation_turns
    """
    now = datetime.now(TIMEZONE)
    manana = now + timedelta(days=1)
    dia_manana = DIAS_ES[manana.weekday()] if manana.weekday() < 7 else DIAS_ES[0]
    ...
    if not incoming:
        # Fallback: take the last human message from existing messages
        for msg in reversed(state.get("messages", [])):
            if isinstance(msg, HumanMessage) or getattr(msg, "type", None) == "human":
                incoming = msg.content
                break
    updates["messages"] = [HumanMessage(content=incoming)]

    # Incrementar turno de conversación
    updates["conversation_turns"] = state.get("conversation_turns", 0) + 1

    # Reset extraction counter (prevent infinite loops across turns)
    updates["_extract_data_calls"] = 0

    # Si hay store, cargar estado previo y merges
    if store:
        try:
            ...
            if prev_state:
                for field in [
                    "patient_name", "selected_service", "service_duration",
                    "intent", "datetime_preference", "patient_phone",
                    "appointment_id", "google_event_id",
                ]:
                    if field in prev_state and not state.get(field):
                        updates[field] = prev_state[field]

            # Cargar historial
            history = await store.get_history(phone, limit=50)
            # El historial se usa para contexto, lo mergeamos con messages
            if history and not state.get("messages"):
                updates["messages"] = history + updates["messages"]
        except Exception as e:
            logger.warning("no se pudo cargar estado previo", error=str(e))

    # Check escalada por número de turns
...
async def node_adjust_weekend(state: ArcadiumState) -> Dict[str, Any]:
    ...
    if adjusted:
        logger.info("Fecha ajustada fin de semana → lunes", original=dt_iso, adjusted=new_iso)
        return {
...
async def node_check_availability(
    ...
    *,
    ...
    if not dt_iso or not calendar_service:
        return {
            "available_slots": [],
            "last_error": "No hay fecha para consultar disponibilidad" if not dt_iso else "Calendar service no disponible",
        }
...
async def node_detect_confirmation(state: ArcadiumState) -> Dict[str, Any]:
    ...
    if result == "slot_choice":
        selected_slot = extract_slot_from_text(
            text, state.get("available_slots", [])
        )
        ...
        if db_service:
            try:
                success, msg, appt = await db_service.create_appointment(
                    session=None,  # Session se maneja fuera del grafo
    ...
    *,
    ...
    try:
        ...
        for k, v in state.items():
            if k == "messages":
                continue  # Store maneja mensajes por separado
            serializable[k] = v

        await store.save_agent_state(phone, serializable)
...
async def node_extract_intent(
    ...
    *,
    ...
    text = _last_human_text(state)
    intent, confidence = await extract_intent_llm(text, llm)
...
async def node_extract_data(
  ...
  1 llamada al LLM.
    ...
    data = await extract_booking_data(text, context, llm)
    ...
    if data.get("service"):
        ...
        if svc_lower in VALID_SERVICES:
            ...
            # Intentar mapear a servicio conocido
            for known, _ in VALID_SERVICES.items():
                if known in svc_lower or svc_lower in known:
                    updates["selected_service"] = known
                    updates["service_duration"] = VALID_SERVICES[known]
                    break
            else:
                updates["selected_service"] = svc_lower
    ...
    logger.info("datos extraídos por LLM", extracted={k: v for k, v in updates.items() if k != "missing_fields"})
    return updates
...
async def node_generate_response(
    ...
    *,
    ...
    text = await generate_deyy_response(context, llm)
